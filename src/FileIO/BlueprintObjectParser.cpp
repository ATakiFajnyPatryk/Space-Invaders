...
string BlueprintObjectParser::extractStringBetweenTags(string stringToSearch,
                                                       string startTag,
                                                       string endTag) {
...
void BlueprintObjectParser::parseNextObjectForBlueprint(ifstream &reader,
  ...
  while (getline(reader, lineFromFile)) {
    if (lineFromFile.find(ObjectTags::COMPONENT) != string::npos) {
      ...
      value = extractStringBetweenTags(lineFromFile, ObjectTags::ENCOMPASSING_RECT_COLLIDER,
